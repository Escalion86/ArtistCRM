...
export default function TasksScreen() {
  ...
  return <Screen><PageHeader title="Задачи" subtitle="Следующие контакты с клиентами" />{allEmpty ? <EmptyState title="Активных задач нет" description="Добавьте следующий контакт в карточке мероприятия." /> : null}<TaskGroup title="Просрочено" items={groups.overdue} danger onComplete={complete} /><TaskGroup title="Сегодня" items={groups.today} onComplete={complete} /><TaskGroup title="Завтра" items={groups.tomorrow} onComplete={complete} /></Screen>
}
const TaskGroup = ({ title, items, danger = false, onComplete }: { title: string; items: Array<{ event: Event; index: number }>; danger?: boolean; onComplete: (event: Event, index: number) => void }) => items.length ? <View style={styles.group}><SectionTitle>{title} · {items.length}</SectionTitle>{items.map(({ event, index }) => { const task = event.additionalEvents?.[index]; return <Pressable key={`${event._id}-${index}`} onPress={() => router.push(`/events/${event._id}` as never)}><Surface><View style={styles.row}><Pressable hitSlop={8} onPress={() => onComplete(event, index)}><MaterialCommunityIcons name="checkbox-blank-circle-outline" size={25} color={danger ? colors.danger : colors.primary} /></Pressable><View style={styles.grow}><Text style={styles.title}>{task?.title || 'Задача'}</Text><Text style={styles.meta}>{event.eventType || 'Мероприятие'} · {task?.date ? new Date(task.date).toLocaleString('ru-RU') : ''}</Text></View><MaterialCommunityIcons name="chevron-right" size={22} color={colors.textMuted} /></View></Surface></Pressable> })}</View> : null
const styles = StyleSheet.create({ group: { gap: spacing.sm }, row: { flexDirection: 'row', alignItems: 'center', gap: spacing.md }, grow: { flex: 1 }, title: { color: colors.text, fontSize: 14, fontWeight: '700' }, meta: { color: colors.textMuted, fontSize: 12, marginTop: 3 } })
