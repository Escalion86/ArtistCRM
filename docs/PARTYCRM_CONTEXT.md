...
- Кабинет исполнителя должен в перспективе показывать назначения из всех компаний пользователя.
...
2. Сделать `PC-MC3`: создание дополнительной компании для уже авторизованного пользователя.
3. Сделать `PC-MC8`: UI-сценарий "создать еще одну компанию".
4. Начать contractor track:
   - `specialization`;
   - `description`;
   - подрядчик без аккаунта;
   - ручная привязка к User.
5. Обновить performer cabinet под назначения из нескольких компаний.
