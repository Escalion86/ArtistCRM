...
const serialize = (proposal, req) => {
  ...
  if (!proposal.publicId || !proposal.publicTokenNonce || !proposal.publicTokenHash) return data
  const origin = process.env.DOMAIN || req.nextUrl.origin
  const publicUrl = buildProposalPublicUrl(proposal, origin)
  return { ...data, publicUrl, renderedMessage: renderProposalMessage(proposal, publicUrl) }
}
...
export const GET = async (req, { params }) => {
  ...
  return NextResponse.json({ success: true, data: serialize(auth.proposal, req) })
}
...
export const PATCH = async (req, { params }) => {
  ...
  if (action === 'publish') {
    if (!auth.proposal.packages?.length) return error('Добавьте хотя бы один вариант', 400, 'packages_required')
    ...
    const previewUrl = buildProposalPublicUrl(auth.proposal, process.env.DOMAIN || req.nextUrl.origin)
    const unresolvedText = [
      renderProposalMessage(auth.proposal, previewUrl),
      ...auth.proposal.blocksSnapshot.flatMap((block) => [block.title, block.text]),
    ].join('\n')
    const unresolved = [...new Set([...unresolvedText.matchAll(/{{\s*([\w.]+)\s*}}/g)].map((match) => match[1]))]
    if (unresolved.length) {
      return error(`Заполните переменные: ${unresolved.join(', ')}`, 400, 'unknown_variables')
    }
    ...
    const selected = auth.proposal.packages.find((item) => item.id === auth.proposal.selectedPackageId)
    if (!selected) return error('Клиент ещё не выбрал вариант', 409, 'selection_required')
    const requestedServiceIds = selected.lines.map((line) => line.serviceId).filter((value) => mongoose.Types.ObjectId.isValid(value))
    ...
    const event = await Events.findOneAndUpdate(
      ...
      { $set: { servicesIds: serviceIds, contractSum: Number(selected.total) || 0 }, $inc: { syncVersion: 1 } },
      { returnDocument: 'after' }
    )
    if (!event) return error('Мероприятие не найдено', 404, 'event_not_found')
    auth.proposal.appliedAt = new Date()
  } else {
    if (auth.proposal.status !== 'draft') return error('Опубликованный снимок нельзя редактировать', 409, 'immutable_snapshot')
    if (body.title !== undefined) {
      const title = String(body.title || '').trim().slice(0, 200)
      if (!title) return error('Укажите название предложения', 400, 'title_required')
      auth.proposal.title = title
    }
    if (body.validUntil !== undefined) {
      const date = new Date(body.validUntil)
      if (Number.isNaN(date.getTime())) return error('Некорректный срок действия', 400, 'bad_valid_until')
      auth.proposal.validUntil = date
    }
    if (body.blocks !== undefined) auth.proposal.blocksSnapshot = normalizeProposalBlocks(body.blocks)
    if (body.packages !== undefined) auth.proposal.packages = normalizeProposalPackages(body.packages)
    if (body.media !== undefined) auth.proposal.mediaSnapshot = normalizeProposalMedia(body.media)
    if (body.messageText !== undefined) auth.proposal.messageText = String(body.messageText || '').trim().slice(0, 4000)
  }
...
export const DELETE = async (_req, { params }) => {
  ...
  const auth = await authorize(id)
  if (auth.response) return auth.response
  if (auth.proposal.status !== 'draft') return error('Можно удалить только черновик', 409, 'published')
