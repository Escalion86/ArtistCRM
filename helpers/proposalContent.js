...
export const DEFAULT_PROPOSAL_BLOCKS = Object.freeze([
  { id: 'cover', type: 'cover', title: 'Персональное предложение', enabled: true },
  { id: 'intro', type: 'intro', title: 'Здравствуйте, {{client.firstName}}!', text: 'Подготовили варианты для вашего мероприятия.', enabled: true },
  { id: 'packages', type: 'packages', title: 'Варианты программы', enabled: true },
  { id: 'media', type: 'media', title: 'Фото и видео', enabled: true },
  { id: 'benefits', type: 'benefits', title: 'Почему выбирают нас', items: [], enabled: true },
  { id: 'terms', type: 'terms', title: 'Условия', text: '', enabled: true },
  { id: 'contacts', type: 'contacts', title: 'Контакты', enabled: true },
  { id: 'cta', type: 'cta', title: 'Выберите подходящий вариант', text: '', enabled: true },
])
...
export const normalizeProposalMedia = (items) => {
  ...
  return items.slice(0, 10).flatMap((item) => {
    ...
    if (!kind || !url || (kind === 'video_link' && !isSupportedVideoLink(url))) return []
    return [{
...
export const normalizeProposalBlocks = (blocks) => {
  const source = Array.isArray(blocks) && blocks.length ? blocks : DEFAULT_PROPOSAL_BLOCKS
  const seen = new Set()
  return source.flatMap((block) => {
    if (!block || !PROPOSAL_BLOCK_TYPES.includes(block.type) || seen.has(block.type)) return []
    seen.add(block.type)
    return [{
      ...
      items: Array.isArray(block.items)
        ? block.items.slice(0, 20).map((item) => clean(item, 500)).filter(Boolean)
        : [],
...
export const normalizeProposalPackages = (packages) => {
  ...
  return packages.slice(0, 6).flatMap((item, index) => {
    ...
    return [{
      ...
      lines,
      total: Math.max(0, Number(item.total) || lines.reduce((sum, line) => sum + line.price, 0)),
...
export const renderProposalVariables = (text, variables) => {
  ...
  const rendered = String(text ?? '').replace(/{{\s*([\w.]+)\s*}}/g, (_, key) => {
...
export const getProposalUnknownVariables = ({ blocks, messageText, variables }) => {
  const unknown = new Set(renderProposalVariables(messageText, variables).unknown)
  normalizeProposalBlocks(blocks).forEach((block) => {
    renderProposalVariables(`${block.title}\n${block.text}`, variables).unknown.forEach((key) => unknown.add(key))
  })
  return [...unknown]
}
